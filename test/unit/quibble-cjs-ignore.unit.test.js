...
describe('quibble cjs ignoreCallsFromThisFile (unit)', function () {
  afterEach(() => quibble.reset())

  it('should work even if used from cjs', async () => {
    const cjsImporingMjs = require('./a-module-ignored')
    ...
    expect(result2.namedFunctionExport()).to.equal('named-function-export')
    ...
    expect(result4.namedFunctionExport()).to.equal('named-function-export')
  })
})
